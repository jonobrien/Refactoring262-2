...
public class Driver
{
	...
	public void endTurn(Player player, int space)
	{
		...
		if (activePlayer == player)
		{
			...
			if (space < 0)
			{
				JOptionPane.showMessageDialog(null, activePlayer.getName()
						+ " made an illegal move", "Invalid Move",
						JOptionPane.INFORMATION_MESSAGE);
			}
			else
			{
				JOptionPane.showMessageDialog(null, activePlayer.getName()
						+ " please make" + " another jump",
	 ...
	 * 
	 * @param type
	 *            the type of player to be created (0 - local, 1 - network)
	 * @param name
