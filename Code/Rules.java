...
public class Rules
{
	...
	public boolean validateMove(Move move)
	{
		...
		try
		{
			...
			// For each piece of the current color, see if there are forced
			// jumps.
			for (int count = 1; count < 64; count++)
			{
				if (theBoard.occupied(count))
				{
					if (theBoard.getPieceAt(count).getColor() == player
							.getColor())
					{
						tempVec = checkForPossibleJumps(count, pieceType,
								player);
						if (!tempVec.isEmpty())
						{
	...
	/**
	 * Will check all the pieces of the opposite player to see if the pieces
	 * that are left can not move.
	 * 
	...
	private boolean checkForNoMoves()
	{
		...
		while (count < 64 && !done)
		{
			...
			if (temp)
			{
				boolean temp2 = theBoard.getPieceAt(count).getColor() != player
						.getColor();
				...
				if (temp && temp2)
				{
					...
					if (theBoard.getPieceAt(count).getColor() == Color.white)
					{
					...
					possibleMoves = checkForPossibleMoves(count, theBoard
							.getPieceAt(count).getType(), player);
	...
	/**
	 * Will check if there are any more pieces left for the player whose turn it
	 * is not.
	 * 
	...
	/**
	 * Will check the board for any jumps that are open to the current player.
	 * If there are any possible jumps the valid end positions will be added to
	 * the vector.
	 * 
	 * @param piecePosition
			...
			Player aPlayer)
	{
		...
		possibleJumps.addAll(wallPieceMoves(piecePosition, true, pieceType,
				player));
		...
		if (pieceType == theBoard.KING)
		{
			...
			for (; i < loop; i++)
			{
								...
								+ adjacentSpots[i])))
				{
					...
					{
						// Check space diagonal to the piece to see if it is
						// empty.
			...
			for (int j = 0; j <= 1; j++)
			{
				if (!leftWallPieces.contains(new Integer(piecePosition
						...
						&& !rightWallPieces.contains(new Integer(piecePosition
								+ adjacentSpots[j])))
				{
					// Check to see if there is a piece this piece can jump
					// over.
					adjacentSpace = theBoard.occupied(piecePosition
							+ adjacentSpots[j]);
					// Get that piece
	...
	/**
	 * After a piece has made a jump, check to see if another jump can be made
	 * with the same piece.
	 * 
	 * @param piecePosition
	 ...
	 */
	private boolean checkForOtherPossibleJump(int piecePosition, int pieceType,
			Player aPlayer)
	{
				...
				for (int i = 0; i <= adjacentSpots.length; i++)
				{
									...
									piecePosition + adjacentSpots[i])))
					{
								...
								&& (aPiece.getColor() != player.getColor()))
						{
							...
							// Check space diagonal to piece to see if its
							// empty.
							endSpace = theBoard.occupied(piecePosition
									+ secondSpots[i]);
			...
			Player aPlayer)
	{
		...
		else
		{
			...
			if (player.getColor() == Color.white
					&& (leftWallPieces.contains(new Integer(piecePosition)) || rightWallPieces
							.contains(new Integer(piecePosition)))
					&& (end == piecePosition - 9))
			{
			...
			Player aPlayer)
	{
		...
		// Get available moves if the piece is on a wall.
		possibleMoves.addAll(wallPieceMoves(piecePosition, false, pieceType,
				player));
				...
				if (leftWallPieces.contains(new Integer(piecePosition)))
				{
					if (jump)
					{
								...
								&& (aPiece.getColor() != player.getColor()))
						{
							...
							if (!endSpace)
							{
								moves.addElement(new Integer(piecePosition - 14));
							}
						}
								...
								&& (aPiece.getColor() != player.getColor()))
						{
							...
							if (!endSpace)
							{
								moves.addElement(new Integer(piecePosition + 18));
							}
						}
					}
